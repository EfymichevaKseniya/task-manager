...
import changesIconService from '../../services/iconAdapter';
...
export const Content: React.ComponentType<ContentType> = ({ type, time }) => {
  const newType = serviceAdapter(type);
  const newIcon = changesIconService(type);
  return (
    <div
      className={`content content__box content__box--${newType.name}`}
      key={newType.id}
    >
      <CustomIcon {...newIcon} />
